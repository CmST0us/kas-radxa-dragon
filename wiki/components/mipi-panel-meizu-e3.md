...
`meta-mipi-panel` 与 `meizu-e3-panel.yml` 是**两个角色**，别混为一谈：

| 角色 | 是什么 | 负责 |
|---|---|---|
| `meta-mipi-panel`（层） | **货架**：承载本 BSP 需支持的全部 MIPI 屏 | 只让各屏**可用**（配方可解析、可被装入），**不**因被引入就启用任何一块 |
| `meizu-e3-panel.yml`（kas 片段） | **选择**：从货架中选中魅族 E3 | 既引入货架层，又在 `local_conf_header` 把魅族 E3 真正接进构建 |

即「引入层」≠「启用屏」。启用某屏的全部接线（`IMAGE_INSTALL` / `KERNEL_MODULE_AUTOLOAD` /
`KERNEL_TECH_DTBO_PROVIDERS` / `MIPI_PANEL_DTBOS`）写在**选择片段**里，不写在 `conf/layer.conf`。
加/换别的屏只需另写一个同类选择片段，不动货架层。

> 历史：`2b3fab3` 及之前曾把这些接线写死在 `conf/layer.conf`（「引入层=启用屏」），
> 与「货架」语义冲突、且多屏共存时会一次性全启用；`56cfec8` 起解耦到选择片段。
...
> 故 dtbo 登记由本层 `recipes-kernel/images/linux-qcom-mergedtb.bbappend` 的 anonymous python
> 读机器配置已设值后增广（recipe 解析在所有 conf 之后，parse 顺序安全，不丢机器原有 dtbo）。
> 该 bbappend **泛化、由选择驱动**：不写死屏名，而是读选择片段填入的 `MIPI_PANEL_DTBOS` 列表逐个并入；
> 未选任何屏时为 no-op。

启用接线分两处（`56cfec8` 起，见上「货架/选择」一节）：
- **选择片段 `meizu-e3-panel.yml` 的 `local_conf_header`**：`IMAGE_INSTALL`、`KERNEL_MODULE_AUTOLOAD`、
  `KERNEL_TECH_DTBO_PROVIDERS`（登记 provider，令 mergedtb depend 其 `do_deploy`）、
  `MIPI_PANEL_DTBOS`（选中 `meizu-e3-panel.dtbo`）——均普通变量 `:append`，用
  `:qcs6490-radxa-dragon-q6a` override 收口（非 Q6A 机器不误装）。
- **货架层 `conf/layer.conf`**：只做集合注册与依赖，**不**含任何具体屏的启用接线。
...
- `meta-mipi-panel`：`github.com/CmST0us/meta-mipi-panel.git`，分支 `scarthgap`，commit `56cfec8`
  （货架/选择解耦后；`main` 与 `scarthgap` 同步指向该 commit。由 `meizu-e3-panel.yml` 远端锁定；
  本地调试切 `path: ../meta-mipi-panel`）。
