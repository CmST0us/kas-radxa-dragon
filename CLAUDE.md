...
## 四·五、openspec 提案规约（写提案前必读）

本仓库用 **openspec**（spec-driven）管理结构化提案。**唯一真值来源是
`openspec/config.yaml`；本节只是入口摘要，与之冲突时以 `config.yaml` 为准。**
对应技能：`openspec-propose / openspec-apply-change / openspec-archive-change /
openspec-sync-specs / openspec-explore`（及 `opsx:*` 别名）。

写 `proposal / design / tasks` 等任何提案制品时，必须遵守：

- **语言**：一律用**中文**撰写。仅「术语」可保留原文——代码标识符、命令、配置项、
  layer/commit、专有名词（kas、Yocto、bitbake、QCS6490 等）；除此之外不得用外文行文。
- **表述**：清晰易懂、不拗口、不含糊。先结论后展开，能用短句说清就不写长句，
  必要时用列表/表格代替大段文字。
- **分层（硬性结构，仅 design）**：由浅入深、逐层递进，依次覆盖
  ① 系统方案（整体目标、边界、与现有系统的关系）→ ② 模块方案（拆哪些模块、各自职责与接口）→
  ③ 流程时序（模块间调用/数据/时序、关键路径）→ ④ 实现细节（具体配置、文件、命令、代码改动）。
  上层不陷入下层细节，下层不复述上层结论；读者可按需停在任一层。
- **实施阶段（tasks）**：通过 **Subagent** 派发，组建「**Agent Work Team**」控制全过程，三角色职责分明：
  - **产品（Product）**：对「最终用户体验成功」负责；
  - **研发（Engineering）**：对「交付物本身」负责；
  - **QA 测试（QA）**：对「最终交付质量」负责。
  研发与产品**密切协作、反复打合**（反复对齐、互相挑战修正），迭代出双方共同认可的最好效果，
  而非一次性交付；研发需具备产品思维。
- **归档到 wiki（硬性）**：提案「最终确认完成」后，必须把内容**总结、归纳后回填进 `wiki/`**
  （新建页或并入已有页），并按本文件「四、工作流」规约在 `wiki/log.md` 追加一条记录。
  别让结论只停留在提案或对话里。
